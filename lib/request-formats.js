...
exports.body = function(data) {
    ...
    response += "<transactionKey>" + data.login + "</transactionKey>";
    response += "</merchantAuthentication>";
...
var buildPaymentInternals = function(val) {
    ...
    if(val.creditCard && typeof val.creditCard == 'object') {
        ...
        if(!val.cardNumber || !val.expirationDate || !val.expirationDate.match(/^[0-9]{4}-[0-9]{2}$/)) {
            return false;
        }
        else {
            response += "<cardNumber>";
            response += val.cardNumber;
            response += "</cardNumber>";
            
            response += "<expirationDate>";
            response += val.expirationDate;
            response += "</expirationDate>";
        ...
        if(val.accountType) {
            if(val.accountType == 'checking' || val.accountType == 'savings' || val.accountType == 'businessChecking') {
                response += "<accountType>";
                response += val.accountType;
                response += "</accountType>";
            }
        }
        
        if(val.routingNumber && val.routingNumber.match(/^[0-9]{9}$/)) {
            response += "<routingNumber>";
            response += val.routingNumber;
            response += "</routingNumber>";
        }
        else {
            return false;
        ...
        if(val.accountNumber && val.accountNumber.match(/^[0-9]{5,17}$/)) {
            response += "<accountNumber>";
            response += val.accountNumber;
            response += "</accountNumber>";
        }
        else {
            return false;
        ...
        if(val.nameOnAccount) {
            response += "<nameOnAccount>";
            response += truncate(22, val.nameOnAccount);
            response += "</nameOnAccount>";
        }
        else {
            return false;
        ...
        if(val.echeckType) {
            if(val.echeckType == 'CCD' || val.echeckType == 'PPD' || val.echeckType == 'TEL' || val.echeckType=='WEB') {
                response += "<echeckType>";
                response += val.echeckType;
                response += "</echeckType>";
            }
            else {
                return false;
        ...
        if(val.bankName) {
            response += "<bankName>";
            response += truncate(50, val.bankName);
            response += "</bankName>";
        }
